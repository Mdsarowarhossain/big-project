...
public class DebugActivity extends Activity {
	...
	private String[] exceptionTypes = { "StringIndexOutOfBoundsException", "IndexOutOfBoundsException",
			"ArithmeticException", "NumberFormatException", "ActivityNotFoundException" };
			...
			try {
				for (int j = 0; j < exceptionTypes.length; j++) {
					if (split[0].contains(exceptionTypes[j])) {
						...
						madeErrorMessage += "Detailed error message:" + errorMessage;
						break;
					}
				}
				...
				madeErrorMessage = madeErrorMessage + "\\Error while getting error: " + Log.getStackTraceString(e);
			}
		}
